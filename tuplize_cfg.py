...
p.output_files = [f'{options.outDir}/{out_name}']
...
fspart = HpstrConf.Processor('fspart','FinalStateParticleProcessor')
...
if (not options.isData):
    p.sequence = [header, vtx, vtxgbl, cvtxgbl, ecal, track, trackgbl, mcpart, fspart]
else:
    p.sequence = [header, vtx, ecal, track, fspart]
